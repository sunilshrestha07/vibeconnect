                ...
                <div className=" w-full sm:aspect-[9/10]  relative" key={ item._id}>
                    ...
                    <div className=" flex items-center gap-3 px-2 py-1">
                        <div className=" w-10 aspect-square  overflow-hidden rounded-full">
                            <img className=' w-full h-full object-cover' src={item.user.avatar} alt="user avatar" />
                        </div>
                        <p className=' text-sm font-semibold text-black'>{item.user.userName}</p>
                        <p className=' text-xs font-semibold text-black'>{moment(item.createdAt).fromNow()}</p>
                    </div>

                    {/* post image */}
                    <div className="w-full aspect-[9/8]  overflow-hidden">
                        <img className='w-full h-full object-cover' src={item.media.url} alt="post image" />
                    </div>
                    ...
                    <div className="">
                        <p className=' font-medium text-base'>{item.user.userName} {item.discription}</p>
                    </div>
